...
def split_dict_list(dict_list):
    ...
    for i, dictionary in enumerate(dict_list):
        # Find the midpoint of the dictionary
        midpoint = len(dictionary) // 2
        
        # Split the dictionary into two halves
        first_half = dict(list(dictionary.items())[:midpoint])
        second_half = dict(list(dictionary.items())[midpoint:])
        
        # Append the halves to respective lists
        list_one.append(first_half)
        list_two.append(second_half)
    
    return list_one, list_two

def split_list(list_u):
    # Find the midpoint of the list
    midpoint = len(list_u) // 2
   
    # Split the list into two halves
    first_half = list_u[:midpoint]
    second_half = list_u[midpoint:]
